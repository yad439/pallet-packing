...
def task_one_model(pallet_width: int, pallet_height: int, items: Collection[Item], *, limit: int = 0,
    ...
    n = len(items)
    zv = [bool(solver.LookupVariable(
        f'z{i}').solution_value()) for i in range(n)]
    ...
    rv = [bool(solver.LookupVariable(
        f'r{i}').solution_value()) for i in range(n)]

    return status, solver.Objective().Value(), solver.Objective().BestBound(), [Position(xv[i], yv[i], rv[i]) if zv[i] else None for i in range(n)]


def task_one_model_adapter(pallet_width: int, pallet_height: int, items: Collection[Item]) -> Tuple[
        int, List[Position]]:
...
# wrong model
def task_two_model(pallet_width: int, pallet_height: int, items: Collection[Item], x_tolerance: Union[float, int],
...
def _build_simple_model(pallet_width: int, pallet_height: int, items: Collection[Item],
    ...
    for i in range(n):
        solver.Add(x[i] + w[i] * (1 - r[i]) + h[i] * r[i]
                   <= pallet_width + (1 - z[i]) * big_num)
        solver.Add(y[i] + h[i] * (1 - r[i]) + w[i] * r[i]
                   <= pallet_height + (1 - z[i]) * big_num)
        for j in range(n):
            if i != j:
                solver.Add(x[i] + w[i] * (1 - r[i]) + h[i] * r[i]
                           <= x[j] + (1 - l[i][j]) * big_num)
    ...
    solver.Minimize(pallet_height * pallet_width -
                    sum(h[i] * w[i] * z[i] for i in range(n)))
    return solver
