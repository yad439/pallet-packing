...
def skyline_decode(pallet_width: int, pallet_height: int, items: List[Item], permutation: Iterable[int]) \
    ...
    while not items_list.is_empty():
        ...
        if score == -1:
            ...
            results[best_item.data[2]] = Position(gap.data[0], gap.data[1], False)

            skyline.insert(gap, (gap.data[0] + best_item.data[0], gap.data[1]))
            gap.data = gap.data[0], gap.data[1] + best_item.data[1]

            items_list.remove(best_item)
        elif score == 1:
            if best_item.data[1] == height1:
                results[best_item.data[2]] = Position(gap.data[0], gap.data[1], False)
                gap.data = gap.data[0] + best_item.data[0], gap.data[1]
            elif best_item.data[1] == height2:
                next_point = gap.next
                if next_point is not None:
                    results[best_item.data[2]] = Position(next_point.data[0] - best_item.data[0], gap.data[1], False)
                    next_point.data = next_point.data[0] - best_item.data[0], next_point.data[1]
                else:
                    results[best_item.data[2]] = Position(pallet_width - best_item.data[0], gap.data[1], False)
                    skyline.add((pallet_width - best_item.data[0], gap.data[1] + best_item.data[1]))
            else:
                results[best_item.data[2]] = Position(gap.data[0], gap.data[1], False)
                gap.data = gap.data[0], gap.data[1] + best_item.data[1]
